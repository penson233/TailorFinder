...
def shufflednsscan(file,outpath,alldomain,platform):
    ...
    if platform == "linux":
        cmd = "cat %s| xargs -I {} ./bin/shuffledns/shuffledns -w ./bin/shuffledns/domain.txt -r ./bin/shuffledns/resolvers.txt -d {} >> %s/shuffledns.txt -m ./bin/massdns/massdns_linux" %(file, outpath)
        runshell(cmd, "shuffledns")
    elif platform =="darwin":
        cmd="cat %s| xargs -I {} ./bin/shuffledns/shuffledns_m -w ./bin/shuffledns/domain.txt -r ./bin/shuffledns/resolvers.txt -d {} -m ./bin/massdns/massdns_mac>> %s/shuffledns.txt" %(file,outpath)
        runshell(cmd, "shuffledns")
